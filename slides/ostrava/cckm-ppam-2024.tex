...
\frame{\frametitle{The primary point of this talk}
  ...
  \end{enumerate}
  \vspace{0.5cm}
  then we may lose the ability to
  \vspace{0.5cm}
  \begin{enumerate}
  \item assert that rounding errors are irrelevant
  \item estimate the discretization error
  \item compare the model against the physical reality
  \end{enumerate}
  \vspace{1cm}
}
...
\frame{\frametitle{Modelling ions: Setup}
  ...
  The total force on an ion is
  \begin{equation}
    \bm{F}(\bm{r_i}) =  - \alpha \sum_{j \not = i} \frac{1}{\|\bm{r}_i - \bm{r}_j\|^3}(\bm{r}_i - \bm{r}_j) - \beta \bm{r_i} - \gamma \bm{v}_i
  \end{equation}
  We measure the total kinetic energy $A_h$ at a fixed time as function of the stepsize $h_k = 2^{-k} h_0$. 
}

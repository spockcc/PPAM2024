...
\frame{\frametitle{The secondary point of this talk}
  ...
  \vspace{0.5cm}
  \begin{enumerate} \setlength\itemsep{1em}
  \item rounding errors are irrelevant
...
\end{equation}
Why is this significant? The computer returns $$\hat{A}_h \not = A_h$$
}
...
\frame{\frametitle{A spectacular failure}
  ...
  We used GROMACS to simulate the motion of hen egg white lysozyme submerged in water.
  \begin{enumerate} \setlength\itemsep{1em}
...
\frame{\frametitle{A spectacular failure}
  ...
  \item We cannot quantifty the effect of changing the force-field.
  \end{itemize}
...
\frame{\frametitle{When do we have an AEX?}
  ...
  Deriving an AEX is an exercise in Taylor expansions. If
  \begin{center}
  ...
  \vspace{1cm}
    \newline
  Every AEX refers to the exact value of the $A_h$. If
  $$\hat{A}_h \approx A_h$$
...
\frame{\frametitle{External ballistics: Accurate event location}

  \begin{figure}
    \includegraphics[width=10cm]{maxrange_rk1_tol53.pdf}
    \caption{RK1: The time to impact is computed with tolerance $\tau = 2^{-53}$. We can identify the asymptotic range and we trust the error estimate.}
  \end{figure}
}

\frame{\frametitle{External ballistics: Inaccurate event location}

  \begin{figure}
    \includegraphics[width=10cm]{maxrange_rk2_tol11.pdf}
    \caption{RK2: The time to impact is computed with tolerance $\tau = 2^{-11}$ and we cannot locate the asymptotic range nor trust the error estimate.}
  \end{figure}
}

\frame{\frametitle{External ballistics: Sufficiently event location}

  \begin{figure}
    \includegraphics[width=10cm]{maxrange_rk2_tol25.pdf}
    \caption{RK2: The time to impact is computed with tolerance $\tau = 2^{-25}$ and the asymptotic range begins to emerge}
  \end{figure}
}

\frame{\frametitle{Molecular dynamics: Truncation of force-fields}

  \begin{itemize}
  \item Simple simulation with 4 ions
  \item Each ion is attached to the origin with a spring
  \item The electrostatic forcefields are trunctated.
  \end{itemize}
}   

...
% Runge-Kutta methods
% The electrostatic forcefields have a jump discontinuity.
% They are truncated at a finite distance.
...
dname=strcat(dpath,'iontrap_mwe2.mat');

% Set the figure path
fpath=strcat(mpath,'fig/');
fname=strcat(fpath,'iontrap_mwe2.eps');
...
if ~isfile(dname)
    ...
    % Set cutoff function
    rho=0.1462/2; rho1=[]; rho2=[]; g2=@(x)cut1(rho,x);
    
    % Set the forcefield: coulomb force + springs + dampning
    f=@(q,p)modified_coulomb_force(dim,n,charge,q,scale,g2)-spring*q-damp*p;

...
%
\begin{abstract} We consider the problem of estimating the error when solving a system of differential algebraic equations. Richardson extrapolation is a classical technique that can be used to judge when rounding errors are irrelevant and estimate the discretization error. We have simulated molecular dynamics with constraints using the GROMACS library and found that the output is not always ameable to Richardson's method. We derive and illustrate Richardson's method using a variety of numerical experiments. We identify two necessary conditions that are not always satisfied by the GROMACS library.
\keywords{}
\end{abstract}
%
%
...
\section{Motivation} Consider the problem resolving the motion of a system of atoms moving in a force-field $\bm{f}$ subject to a set of constraints $g$. In this case, Newton's 2nd law takes the form the following system of differential algebraic equations
\begin{align}
  \bm{q}'(t) &= \bm{v}(t) \\
...
\end{equation}
We conclude that $T_0 - T(\bm{f})$ is small, if the computational error $A_h(\bm{f}) - A_h(\bm{f}, \tau, u)$ and the discretization error $T(\bm{f}) - A_h(\bm{f})$ are both small. If we cannot assert control over these errors, then we cannot say with certainty that our model delivers a good approximation of the physical reality.
...
Our goal has been the efficient reduction of the computational error in order to improve the long-term stability of constrained simulations of molecular dynamics at low cost.
...
Thirdly, if the computed energies for ${\tt tol} = 10 ^{-12}$ follow an asymptotic error expansion, then the commonly used time step of $1$ fs ($n=1000$ in this case) is \emph{not} well inside the asymptotic range. Why is this? If we where in the asymptotic range, then the error $T - \hat{A}_h$ would very nearly obey a power law and $\hat{A}_h \approx T - \alpha h^p$ would be a good approximation for some power $p>0$. In particular, the $\hat{A}_h$  would behave in be \emph{monotone} manner and the tiny oscillations that we have recorded would not be present.
...
\begin{figure}
  \includegraphics[width=\linewidth]{maxrange_rk2_tol25.pdf} \caption{The evolution of $F_h$ for the maximum range of 6 different shells computed using Heyn's method (\text{'rk2'}) and $\text{tol} = 2^{-25}$.}
...
Let $\bm{f}$ denote the forcefield generated by an ion located at $0$ with charge $q$. Then
\begin{equation}
...
The scripts {\tt iontrap\_mwe2} and {\tt iontrap\_mwe4} replace $f$ with
\begin{equation}
...
When the force-fields are not perturbed, Figure~\ref{fig:iontrap_mwe1}, the experiments supports the conjecture that the exists an asymptotic error expansion and we can clearly identify an asymptotic range for each of the 4 Runge-Kutta methods used to integrate the dynamical system. 
...
When the force-fields are truncated and jump-discontinuities are introduced, Figure~\ref{fig:iontrap_mwe2}, there is no evidence to suggest that there exists an asymptotic error expansion and there is no reason to suspect that Richardson's error estimate is accurate. We cannot assert that the computational error is irrelevant and without an estimate of the discretization error we cannot quantify the impact of changing the force-field.
